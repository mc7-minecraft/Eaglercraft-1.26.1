...
public class ItemInHandRenderer {
   ...
   private void applyEatTransform(final PoseStack poseStack, final float frameInterp, final HumanoidArm arm, final ItemStack itemStack, final Player player) {
      ...
      poseStack.mulPose(Axis.YP.rotationDegrees((float)invert * eatJiggle * 90.0F));
      poseStack.mulPose(Axis.XP.rotationDegrees(eatJiggle * 10.0F));
      poseStack.mulPose(Axis.ZP.rotationDegrees((float)invert * eatJiggle * 30.0F));
   }
   ...
   private void applyItemArmAttackTransform(final PoseStack poseStack, final HumanoidArm arm, final float attackValue) {
      ...
      poseStack.mulPose(Axis.YP.rotationDegrees((float)invert * (45.0F + ySwingRotation * -20.0F)));
      float xzSwingRotation = Mth.sin((double)(Mth.sqrt(attackValue) * (float) Math.PI));
      poseStack.mulPose(Axis.ZP.rotationDegrees((float)invert * xzSwingRotation * -20.0F));
      ...
      if (!player.isScoping()) {
         ...
         if (itemStack.isEmpty()) {
            ...
            if (player.isUsingItem() && player.getUseItemRemainingTicks() > 0 && player.getUsedItemHand() == hand && !charged) {
               ...
               poseStack.mulPose(Axis.YP.rotationDegrees((float)invert * 65.3F));
               ...
               if (charged && attack < 0.001F && isMainHand) {
                  ...
                  poseStack.mulPose(Axis.YP.rotationDegrees((float)invert * 10.0F));
               }
            }
            ...
            if (player.isUsingItem() && player.getUseItemRemainingTicks() > 0 && player.getUsedItemHand() == hand) {
               ...
               switch (useAnimation) {
                  ...
                  case BOW:
                     ...
                     poseStack.mulPose(Axis.ZP.rotationDegrees((float)invert * -9.785F));
                     float timeHeldxxx = (float)itemStack.getUseDuration(player) - ((float)player.getUseItemRemainingTicks() - frameInterp + 1.0F);
                  ...
                  case TRIDENT:
                     ...
                     poseStack.mulPose(Axis.YP.rotationDegrees((float)invert * 35.3F));
                     poseStack.mulPose(Axis.ZP.rotationDegrees((float)invert * -9.785F));
                     float timeHeldxx = (float)itemStack.getUseDuration(player) - ((float)player.getUseItemRemainingTicks() - frameInterp + 1.0F);
                  ...
                  case SPEAR:
                     ...
                     float timeHeldx = (float)itemStack.getUseDuration(player) - ((float)player.getUseItemRemainingTicks() - frameInterp + 1.0F);
               ...
               poseStack.mulPose(Axis.YP.rotationDegrees((float)invert * 65.0F));
               poseStack.mulPose(Axis.ZP.rotationDegrees((float)invert * -85.0F));
            } else {
               this.applyItemArmTransform(poseStack, arm, inverseArmHeight);
   ...
   private void swingArm(final float attack, final PoseStack poseStack, final int invert, final HumanoidArm arm) {
      ...
      float ySwingPosition = 0.2F * Mth.sin((double)(Mth.sqrt(attack) * (float) (Math.PI * 2)));
