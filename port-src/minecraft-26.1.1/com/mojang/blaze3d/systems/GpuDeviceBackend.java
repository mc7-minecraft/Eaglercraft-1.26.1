...
public interface GpuDeviceBackend {
   ...
   GpuTexture createTexture(
      @Nullable Supplier<String> label, @Usage final int usage, TextureFormat format, int width, int height, int depthOrLayers, int mipLevels
   );

   GpuTexture createTexture(@Nullable String label, @Usage final int usage, TextureFormat format, int width, int height, int depthOrLayers, int mipLevels);
   ...
   GpuBuffer createBuffer(@Nullable Supplier<String> label, @com.mojang.blaze3d.buffers.GpuBuffer.Usage int usage, long size);

   GpuBuffer createBuffer(@Nullable Supplier<String> label, @com.mojang.blaze3d.buffers.GpuBuffer.Usage int usage, ByteBuffer data);
