...
public class VanillaHusbandryAdvancements implements AdvancementSubProvider {
   ...
   public void generate(final HolderLookup.Provider registries, final Consumer<AdvancementHolder> output) {
      ...
      Advancement.Builder.advancement()
         ...
         .addCriterion(
            "kill_axolotl_target", EffectsChangedTrigger.TriggerInstance.gotEffectsFrom(EntityPredicate.Builder.entity().of(entityTypes, EntityType.AXOLOTL))
         )
         .display(
   ...
   private static Advancement.Builder addFishBuckets(final Advancement.Builder advancement, final HolderGetter<Item> items) {
      for (Item bucket : FISH_BUCKETS) {
         advancement.addCriterion(
            BuiltInRegistries.ITEM.getKey(bucket).getPath(), FilledBucketTrigger.TriggerInstance.filledBucket(ItemPredicate.Builder.item().of(items, bucket))
         );
      }
         ...
         .forEach(
            v -> advancement.addCriterion(
                  ...
                  TameAnimalTrigger.TriggerInstance.tamedAnimal(
                     EntityPredicate.Builder.entity()
                        .components(DataComponentMatchers.Builder.components().exact(DataComponentExactPredicate.expect(DataComponents.CAT_VARIANT, v)).build())
                  )
               )
         );
      return advancement;
