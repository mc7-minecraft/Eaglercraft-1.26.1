         ...
         return TypeRewriteRule.seq(
            this.fixTypeEverywhere(
               ...
               ops -> input -> {
                     ...
                     while (true) {
                        Either<List<NewEntityTree>, Unit> passengersValue = (Either<List<NewEntityTree>, Unit>)DataFixUtils.orElse(
                           passenger.map(
                              p -> {
                                 Typed<NewEntityTree> newEntity = (Typed<NewEntityTree>)newEntityTreeType.pointTyped(ops)
                                    .orElseThrow(() -> new IllegalStateException("Could not create new entity tree"));
                                 NewEntityTree newEntityTree = (NewEntityTree)newEntity.set(newEntityTreeValueFinder, p)
                        ...
                        Entity ridingEntity = (Entity)((Pair)updating.getSecond()).getSecond();
                        passenger = Optional.of(
                           Pair.of(References.ENTITY_TREE.typeName(), Pair.of(passengersValue, ridingEntity))
                        );
