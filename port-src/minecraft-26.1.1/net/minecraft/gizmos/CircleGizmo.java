...
public record CircleGizmo(Vec3 pos, float radius, GizmoStyle style) implements Gizmo {
   ...
   public void emit(final GizmoPrimitives primitives, final float alphaMultiplier) {
      if (this.style.hasStroke() || this.style.hasFill()) {
         ...
         for (int i = 0; i < 20; i++) {
            float theta = (float)i * (float) (Math.PI / 10);
            Vec3 point = this.pos
               .add((double)((float)((double)this.radius * Math.cos((double)theta))), 0.0, (double)((float)((double)this.radius * Math.sin((double)theta))));
            points[i] = point;
         }
