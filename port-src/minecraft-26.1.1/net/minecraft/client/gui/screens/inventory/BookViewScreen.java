...
public class BookViewScreen extends Screen {
   ...
   protected boolean handleClickEvent(@Nullable final ClickEvent event) {
      if (event == null) {
         ...
         if (event instanceof ClickEvent.ChangePage changePage) {
            this.forcePage(changePage.page() - 1);
            return true;
         }

         if (event instanceof ClickEvent.RunCommand runCommand) {
            this.closeContainerOnServer();
            clickCommandAction(player, runCommand.command(), null);
            return true;
         }
   ...
   public static record BookAccess(List<Component> pages) {
      ...
      public static BookViewScreen.BookAccess fromItem(final ItemStack itemStack) {
         ...
         if (writtenContent != null) {
            ...
            return writableContent != null
               ? new BookViewScreen.BookAccess(writableContent.getPages(filterEnabled).map(page -> (Component)Component.literal(page)).toList())
               : null;
         }
      }
   }
