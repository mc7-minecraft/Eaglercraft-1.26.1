   ...
   protected void renderToTexture(final GuiProfilerChartRenderState chartState, final PoseStack poseStack) {
      ...
      for (ResultField result : chartState.chartData()) {
         ...
         for (int j = steps; j >= 0; j--) {
            float dir = (float)((totalPercentage + result.percentage * (double)j / (double)steps) * (float) (Math.PI * 2) / 100.0);
         ...
         for (int j = steps; j > 0; j--) {
            float dir0 = (float)((totalPercentage + result.percentage * (double)j / (double)steps) * (float) (Math.PI * 2) / 100.0);
            float x0 = Mth.sin((double)dir0) * 105.0F;
            float y0 = Mth.cos((double)dir0) * 105.0F * 0.5F;
            float dir1 = (float)((totalPercentage + result.percentage * (double)(j - 1) / (double)steps) * (float) (Math.PI * 2) / 100.0);
