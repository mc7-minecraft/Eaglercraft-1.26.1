...
public enum GraphicsPreset implements StringRepresentable
{
    ...
    public void apply(Minecraft minecraft) {
        ...
        switch (this.ordinal()) {
            case 0: {
                int viewDistance = 6;
                this.set(screen, minecraft.options.biomeBlendRadius(), 0);
                this.set(screen, minecraft.options.renderDistance(), 6);
                this.set(screen, minecraft.options.prioritizeChunkUpdates(), PrioritizeChunkUpdates.NONE);
                this.set(screen, minecraft.options.simulationDistance(), 5);
                this.set(screen, minecraft.options.ambientOcclusion(), false);
                this.set(screen, minecraft.options.cloudStatus(), CloudStatus.OFF);
                this.set(screen, minecraft.options.particles(), ParticleStatus.MINIMAL);
                this.set(screen, minecraft.options.mipmapLevels(), 0);
                this.set(screen, minecraft.options.entityShadows(), false);
                this.set(screen, minecraft.options.entityDistanceScaling(), 0.5);
                this.set(screen, minecraft.options.menuBackgroundBlurriness(), 0);
                this.set(screen, minecraft.options.cloudRange(), 8);
                this.set(screen, minecraft.options.cutoutLeaves(), false);
                this.set(screen, minecraft.options.improvedTransparency(), false);
                this.set(screen, minecraft.options.weatherRadius(), 3);
