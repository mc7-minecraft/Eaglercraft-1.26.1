...
public class SurfaceSystem {
   ...
   private void erodedBadlandsExtension(final BlockColumn column, final int blockX, final int blockZ, final int height, final LevelHeightAccessor protoChunk) {
      ...
      double pillarBuffer = Math.min(
         Math.abs(this.badlandsSurfaceNoise.getValue((double)blockX, 0.0, (double)blockZ) * 8.25),
         this.badlandsPillarNoise.getValue((double)blockX * 0.2, 0.0, (double)blockZ * 0.2) * 15.0
      );
      if (!(pillarBuffer <= 0.0)) {
   ...
   private void frozenOceanExtension(
      ...
      double iceberg = Math.min(
         Math.abs(this.icebergSurfaceNoise.getValue((double)blockX, 0.0, (double)blockZ) * 8.25),
         this.icebergPillarNoise.getValue((double)blockX * 1.28, 0.0, (double)blockZ * 1.28) * 15.0
      );
      if (!(iceberg <= 1.8)) {
