...
public class Lighting implements AutoCloseable {
   ...
   public Lighting() {
      ...
      this.buffer = device.createBuffer(() -> "Lighting UBO", 136, this.paddedSize * (long)com.mojang.blaze3d.platform.Lighting.Entry.values().length);
      Matrix4f flatPose = new Matrix4f().rotationY((float) (-Math.PI / 8)).rotateX((float) (Math.PI * 3.0 / 4.0));
      this.updateBuffer(
         com.mojang.blaze3d.platform.Lighting.Entry.ITEMS_FLAT,
      ...
      this.updateBuffer(
         com.mojang.blaze3d.platform.Lighting.Entry.ITEMS_3D,
         item3DPose.transformDirection(DIFFUSE_LIGHT_0, new Vector3f()),
         item3DPose.transformDirection(DIFFUSE_LIGHT_1, new Vector3f())
      );
      this.updateBuffer(com.mojang.blaze3d.platform.Lighting.Entry.ENTITY_IN_UI, INVENTORY_DIFFUSE_LIGHT_0, INVENTORY_DIFFUSE_LIGHT_1);
      Matrix4f playerSkinPose = new Matrix4f();
      this.updateBuffer(
         com.mojang.blaze3d.platform.Lighting.Entry.PLAYER_SKIN,
   ...
   public void updateLevel(final Type type) {
      switch (type) {
         case DEFAULT:
            this.updateBuffer(com.mojang.blaze3d.platform.Lighting.Entry.LEVEL, DIFFUSE_LIGHT_0, DIFFUSE_LIGHT_1);
            break;
         case NETHER:
            this.updateBuffer(com.mojang.blaze3d.platform.Lighting.Entry.LEVEL, NETHER_DIFFUSE_LIGHT_0, NETHER_DIFFUSE_LIGHT_1);
      }
   }

   private void updateBuffer(final com.mojang.blaze3d.platform.Lighting.Entry entry, final Vector3f light0, final Vector3f light1) {
   ...
   public void setupFor(final com.mojang.blaze3d.platform.Lighting.Entry entry) {
