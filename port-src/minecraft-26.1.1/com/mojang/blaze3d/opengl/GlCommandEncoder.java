...
class GlCommandEncoder implements CommandEncoderBackend {
      ...
      if (indexType != null) {
         ...
         if (instanceCount > 1) {
            ...
            GL32.glDrawElementsBaseVertex(
               GlConst.toGl(pipeline.info().getVertexFormatMode()), drawCount, GlConst.toGl(indexType), (long)firstIndex * (long)indexType.bytes, baseVertex
            );
         } else {
            GlStateManager._drawElements(
               GlConst.toGl(pipeline.info().getVertexFormatMode()), drawCount, GlConst.toGl(indexType), (long)firstIndex * (long)indexType.bytes
            );
         }
   ...
   private boolean trySetup(final GlRenderPass renderPass, final Collection<String> dynamicUniforms) {
      ...
      for (Entry<String, Uniform> entryx : glProgram.getUniforms().entrySet()) {
         ...
         Uniform uniform = Objects.requireNonNull(entryx.getValue());

         if (uniform instanceof Uniform.Ubo ubo) {
            if (isDirty) {
               GpuBufferSlice bufferView = renderPass.uniforms.get(namex);
               GL32.glBindBufferRange(35345, ubo.blockBinding(), ((GlBuffer)bufferView.buffer()).handle, bufferView.offset(), bufferView.length());
            }
            continue;
         }

         if (uniform instanceof Uniform.Utb utb) {
            int location = utb.location();
            int samplerIndex = utb.samplerIndex();
            ...
            GL11C.glBindTexture(35882, utb.texture());
            if (isDirty) {
               GpuBufferSlice bufferView = renderPass.uniforms.get(namex);
               GL31.glTexBuffer(35882, GlConst.toGlInternalId(utb.format()), ((GlBuffer)bufferView.buffer()).handle);
            }
            continue;
         }
         ...
         if (uniform instanceof Uniform.Sampler sampler) {
            ...
            int samplerIndex = sampler.samplerIndex();
            GlRenderPass.TextureViewAndSampler viewAndSampler = renderPass.samplers.get(namex);
            if (viewAndSampler == null) {
               continue;
            }
