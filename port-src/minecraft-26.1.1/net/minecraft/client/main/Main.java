...
public class Main {
   public static void main(final String[] args) {
      ...
      try {
         ...
         String configuredUsername = firstNonBlank(System.getProperty("eaglercraft.username"), (String)usernameOption.value(optionSet));
         String configuredUuid = firstNonBlank(
            System.getProperty("eaglercraft.uuid"),
            optionSet.has(uuidOption) ? (String)uuidOption.value(optionSet) : null
         );
         UUID uuid = isUuidValid(configuredUuid, logger) ? UndashedUuid.fromStringLenient(configuredUuid) : UUIDUtil.createOfflinePlayerUUID(configuredUsername);
         ...
         if (!quickPlayVariant.isEnabled()) {
            String eagerJoinServer = firstNonBlank(System.getProperty("eaglercraft.joinServer"), System.getProperty("eaglercraft.server"));
            if (stringHasValue(eagerJoinServer)) {
               quickPlayVariant = new GameConfig.QuickPlayMultiplayerData(eagerJoinServer);
            }
         }
         User user = new User(
            configuredUsername,
   ...
   @Nullable
   private static String firstNonBlank(@Nullable final String a, @Nullable final String b) {
      if (stringHasValue(a)) {
         return a;
      }

      return stringHasValue(b) ? b : null;
   }
   ...
   private static boolean isUuidValid(@Nullable final String uuid, final Logger logger) {
      if (!stringHasValue(uuid)) {
         return false;
      }

      try {
         UndashedUuid.fromStringLenient(uuid);
         return true;
