...
public class MacroFunction<T extends ExecutionCommandSource<T>> implements CommandFunction<T> {
   ...
   private static String stringify(final Tag tag) {
      ...
      if (tag instanceof FloatTag floatTag) {
         return DECIMAL_FORMAT.format((double)floatTag.value());
      } else if (tag instanceof DoubleTag doubleTag) {
         return DECIMAL_FORMAT.format(doubleTag.value());
      } else if (tag instanceof ByteTag byteTag) {
         return String.valueOf((int)byteTag.value());
      } else if (tag instanceof ShortTag shortTag) {
         return String.valueOf((int)shortTag.value());
      } else if (tag instanceof LongTag longTag) {
         return String.valueOf(longTag.value());
      } else {
         return tag instanceof StringTag stringTag ? stringTag.value() : tag.toString();
      }
   }
