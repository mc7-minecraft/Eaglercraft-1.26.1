...
public class TracyFrameCapture implements AutoCloseable {
   ...
   public void capture(final RenderTarget captureTarget) {
      if (this.status == Status.WAITING_FOR_CAPTURE && !this.capturedThisFrame && captureTarget.getColorTexture() != null) {
         ...
         commandEncoder.copyTextureToBuffer(this.frameBuffer, this.pixelbuffer, 0L, () -> this.status = Status.WAITING_FOR_UPLOAD, 0);
   ...
   public void upload() {
      if (this.status == Status.WAITING_FOR_UPLOAD) {
         ...
         MappedView view = RenderSystem.getDevice().createCommandEncoder().mapBuffer(this.pixelbuffer, true, false);
